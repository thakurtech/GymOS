...
const Sidebar = ({ sidebarCollapsed}) => {
    const sidebarItems = [
        { icon: <IoIosHome />, text: 'Dashboard',link:'dashboard' },
        { icon: <PiUsersThreeFill />, text: 'Calorie Tracker',link:'calorie-tracker' },
        {icon: <PiUsersThreeFill />, text: 'Your Diet',link:'your-diet'},
        { icon: <GoGraph />, text: 'Diet Recommendation',link:'diet-recommendation' },
        {icon: <PiUsersThreeFill />, text: 'Payment',link:'payment'}
    ];
                    ...
                    <ul className="flex flex-col gap-1 mt-2">
                        {sidebarItems.map((item, index) => (
                            <li key={index} className="text-gray-500 hover:bg-gray-100 hover:text-gray-900">
                                <a className='w-full flex items-center px-4 py-3 ' href={item.link}>
                                    {item.icon}
